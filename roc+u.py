...
def create_roc_with_outliers(num_cliques, clique_size):
    ...
    for i in range(num_cliques):
        ...
        labels[outlier_node] = -1  # Outliers marked as -1
...
nodes = nx.draw_networkx_nodes(G_roc, pos, node_color=community_colors, cmap=plt.cm.tab10, node_size=300)
...
sm = plt.cm.ScalarMappable(cmap=plt.cm.tab10, norm=plt.Normalize(vmin=-1, vmax=num_cliques - 1))
sm.set_array([])
plt.colorbar(sm, ticks=range(-1, num_cliques), label="Ground Truth Community", ax=plt.gca())

plt.title("Ring of Cliques (ROC) with Outliers as Bridges")
